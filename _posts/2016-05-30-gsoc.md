...
<div lang="latex">
where \begin{center}\hspace{-5em} {$s_{i}...s_{n}$} is a letter sequence in the source script, and \\ {$t_{i}...t_{n}$} is the corresponding letter sequence in the target script.\end{center}
</div>

Instead of maximum likelihood estimates, I use [structured perceptron of Collins](http://www.aclweb.org/anthology/W02-1001) to learn the model parameters. With structured perceptron local contextual features can be made relevant to the whole sequence of target letters. It also allows us to use feature based emissions. I replace the basic multinomial emissions \e{P(s$_{i}$}|\e{t$_{i}$)} with the feature based emissions \e{($\theta$ $\cdot$ f(s,t))}; where \e{f(s,t)} is a feature function and $\theta$ are the model parameters. The feature template used to learn the emissions is shown in Table (\ref{tbl:trans}). I use second-order viterbi to decode the best letter sequence in the target script while learning the parameters as well as at the time of testing.
