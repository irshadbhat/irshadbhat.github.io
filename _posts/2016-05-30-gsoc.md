...
#### Word alighnments with GIZA++
 ...
 2. Run the following bash script to word-align the parallel text files (say ***hin.txt*** and ***eng.txt***):
    ...
    Note that ***map_table*** is a python dictionary with Hindi letters as keys and their Roman mappings as values. The edit-distance returned is unbound and can range between 0 to the length of largest string (source and target). These edit-distances are normalized between 0 to 1 simply by dividing the returned distance with the length of the largest string. An edit-distance of 1 between two strings with max-string length of 2 has a normalized edit-distance of 0.5, whereas the same edit distance between two strings with max-string length of 10 has a normalized edit distance of 0.1. 

[comment]: <>     The transliteration pairs are sorted by their edit-distances and a threshold is selected to discriminate between 
