...
Instead of maximum likelihood estimates, I use [structured perceptron of Collins](http://www.aclweb.org/anthology/W02-1001) to learn the model parameters. With structured perceptron local contextual features can be made relevant to the whole sequence of target letters. It also allows us to use feature based emissions. I replace the basic multinomial emissions <span lang="latex">P(s$_{i}$}\|\e{t$_{i}$)</span> with the feature based emissions <div lang="latex">($\theta$ $\cdot$ f(s,t))</div>; where *f(s,t)* is a feature function and \(\theta\) are the model parameters. The feature template used to learn the emissions is shown in Table (\ref{tbl:trans}). I use second-order viterbi to decode the best letter sequence in the target script while learning the parameters as well as at the time of testing.
