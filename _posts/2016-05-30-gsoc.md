...
#### Word alighnments with GIZA++
 ...
 2. Run the following bash script to word-align the parallel text files (say ***hin.txt*** and ***eng.txt***):
    ...
    ```
    # Sentence pair (1) source length 9 target length 7 alignment score : 1.94585e-17
    साफ पानी अधिक से अधिक पीयें ।
    NULL ({ }) Drink ({ 1 6 }) pure ({ }) water ({ 2 }) as ({ }) much ({ 3 5 }) as ({ 4 }) you ({ }) can ({ }) . ({ 7 })
    # Sentence pair (2) source length 6 target length 6 alignment score : 2.28771e-11
    लगातार बुखार से पीड़ित हो ?
    NULL ({ }) I ({ }) suffer ({ 4 5 }) from ({ 3 }) fever ({ 2 }) continuously ({ 1 }) . ({ 6 })
    # Sentence pair (3) source length 10 target length 11 alignment score : 2.70124e-12
    कालाजार , मलेरिया या फिर हाई फीवर हो सकता है ।
    NULL ({ }) Kalajar ({ 1 }) , ({ 2 }) Malaria ({ 3 }) or ({ 4 }) high ({ }) fiver ({ 5 6 7 }) may ({ 9 }) have ({ 10 }) happened ({ 8 }) . ({ 11 })
    # Sentence pair (4) source length 8 target length 9 alignment score : 8.33816e-16
    हर छह माह में करें आँखों की जाँच ।
    NULL ({ 4 }) Get ({ 5 }) eyes ({ 6 }) checked ({ 7 8 }) up ({ }) every ({ 1 }) six ({ 2 }) months ({ 3 }) . ({ 9 })
    # Sentence pair (5) source length 4 target length 4 alignment score : 1.89822e-07
    नियमित आँखें धोना ।
