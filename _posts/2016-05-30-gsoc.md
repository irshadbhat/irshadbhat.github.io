...
Translation pairs with a normalized score of less than a small threshold of **~0.3** are considered as transliteration pairs. Note that the threshold is choosen by a rough observation of translation pairs and can vary for different language pairs.

#### Character alighnments with GIZA++
In order to train the transliteration system, we need to caharacter align the transliteration pairs obtained above. We again use GIZA++ for this task. The resulting character alignments obtained using GIZA++ will look like:

```
# Sentence pair (1) source length 6 target length 10 alignment score : 3.91859e-13
m i s s i o n a r y
NULL ({ 5 6 8 }) m ({ 1 }) i ({ 2 }) Sa ({ 3 4 }) na ({ 7 }) r ({ 9 }) I ({ 10 })
# Sentence pair (2) source length 8 target length 6 alignment score : 1.5955e-05
u n i t e d
NULL ({ }) y ({ }) U ({ 1 }) n ({ 2 }) A ({ }) i ({ 3 }) t ({ 4 }) e ({ 5 }) da ({ 6 })
# Sentence pair (3) source length 6 target length 8 alignment score : 9.23754e-05
a i sh w a r y a
NULL ({ 1 5 }) E ({ 2 }) S ({ 3 }) va ({ 4 }) r ({ 6 }) y ({ 7 }) A ({ 8 })
# Sentence pair (4) source length 6 target length 8 alignment score : 0.0111937
n a r e n d r a
NULL ({ 2 8 }) na ({ 1 }) r ({ 3 }) e ({ 4 }) M ({ 5 }) x ({ 6 }) ra ({ 7 })
.
.
.
```

Giza++ produces  four types of alignments: 1-to-1, 1-to-Many any and 1-to-None and None-to-1. Out of these four letter alignments, None-to-1 alignments need to be handled. These alignments are modeifed by merging the target character with the previous aligned pair if it is not the first character, otherwise it is merged with the succeeding aligned character pair. The final training data looks like:
